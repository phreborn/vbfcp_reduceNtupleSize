    ...
    for(auto mc : lumi){
      ...
      for(auto f : subFiles){
        ...
        for(auto sys : sysList){
          ...
          string cuts = Form("%s.isPassed && !isDalitz && %s.N_j_30>1 && (%s.DeltaEta_jj<-2 || %s.DeltaEta_jj>2) && (%s.Zepp>-5 && %s.Zepp<5)", sysName, sysName, sysName, sysName, sysName, sysName); cout<<"cuts : "<<endl<<cuts<<endl;
